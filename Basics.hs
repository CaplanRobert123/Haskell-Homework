...
data Cell = Hunter |NewTarget Target | Obstacle | Gateway | Empty
    deriving (Eq, Ord)
...
instance Show Cell
    where 
        show Hunter = "!"
        show Empty = " "
        show (NewTarget (Target pos behavior)) = "*"
        show Gateway = "#"
        show Obstacle = "@"

gameAsString :: Game -> String
gameAsString (Game rows columns board) = intercalate "\n" line
    where line = [concatMap show (take columns (drop x (board))) | x <- [0, columns..columns*(rows-1)]]
